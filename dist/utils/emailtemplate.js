...
const totalDiscount = (order) => {
    return order.reduce((sum, item) => sum + (item.price * item.quantity * item.discount) / 100, 0);
};
const totalAmount = (order) => {
    return order.reduce((sum, item) => sum + item.price * item.quantity, 0);
};
const totalPayable = (order, shippingCharge) => {
    return (order.reduce((sum, item) => sum + item.price * item.quantity, 0) -
        totalDiscount(order) +
        shippingCharge);
};
const emailTemplate = (order, getOrder, firstName, lastName) => `
  <!DOCTYPE html>
  <html>
   <style>
body {
    ...
    .map((item, index) => `
            <tr>
              ...
              <td style="border: 1px solid #ddd; padding: 8px;">৳${(item.price * item.quantity * item.discount) / 100}</td>
            </tr>
          `)
    .join('')}
        </tbody>
